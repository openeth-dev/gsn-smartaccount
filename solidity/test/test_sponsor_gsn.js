...
contract('GSN and Sponsor integration', async function (accounts) {
  ...
  before(async function () {
    ...
    gatekeeper = await Gatekeeper.new(bypassLib.address, { gas: 8e6 })
    await gatekeeper.ctr2(gsnForwarder.address, accounts[0], bypassLib.address)
  ...
  it('should accept a relayed call if it comes from a valid participant', async function () {
    const calldata = gatekeeper.contract.methods.changeConfiguration(operatorA.permLevel, actions, args, args,
      await nonce()).encodeABI()
    ...
    const result = await gatekeeper.acceptRelayedCall(relayServer.address, operatorA.address, calldata, 0, 0, 0, 0, [],
      0)
  ...
  it('should reject a relayed call if it doesn\'t come from a participant', async function () {
    ...
    // I know that gas-related params do not matter, so there is no need to test them now
    const result = await gatekeeper.acceptRelayedCall(relayServer.address, nonParticipant.address, calldata, 0, 0, 0, 0,
      [], 0)
